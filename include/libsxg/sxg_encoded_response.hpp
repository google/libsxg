...
class EncodedResponse {
 public:
  ...
  static EncodedResponse Encode(size_t mi_record_size, const RawResponse& src);

  std::string GetHeaderIntegrity() const;

  size_t HeaderSize() const { return header_.Size(); }

  const sxg::Header& GetHeader() const { return header_; }
